...
class _SignUpScreenState extends State<SignUpScreen> {
  ...
  void _handleSignUp() async {
    ...
    try {
      final responseData = await ApiService.instance.signup(
        name: name,
        email: email,
        password: password,
        phone: phone,
      ...
      // Extract OTP code if returned in signup API response
      String? initialOtp;
      if (responseData is Map) {
        if (responseData.containsKey('code')) {
          initialOtp = responseData['code']?.toString();
        } else if (responseData.containsKey('otp')) {
          initialOtp = responseData['otp']?.toString();
        } else if (responseData.containsKey('data') && responseData['data'] is Map) {
          initialOtp = responseData['data']['code']?.toString() ?? responseData['data']['otp']?.toString();
        }
      }

      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(initialOtp != null ? 'OTP received ($initialOtp)!' : 'OTP sent to your email!'),
          backgroundColor: Colors.green,
        ),
      );

      // Go to Verification Screen, pass email, name, purpose & initialOtp
      Navigator.push(
        context,
        MaterialPageRoute(
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: SingleChildScrollView(
        child: Stack(
          children: [
            // 1. Background Orange Header Container with wave elements
            Container(
